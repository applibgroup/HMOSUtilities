...
public class ColorHelper {
	...
	public static int blendColors(final int color1, final int color2, final float ratio) {
		...
		rgbColor.setRed(color1);
		rgbColor.setGreen(color1);
		rgbColor.setBlue(color1);
		RgbColor rgbColor1=new RgbColor();
		rgbColor1.setRed(color2);
		rgbColor1.setGreen(color2);
		rgbColor1.setBlue(color2);
		final float inverseRatio = 1f - ratio;

		final float a = (Color.alpha(color1) * inverseRatio) + (Color.alpha(color2) * ratio);
		final float r = (rgbColor.getRed() * inverseRatio) + (rgbColor1.getRed() * ratio);
		final float g = (rgbColor.getGreen() * inverseRatio) + (rgbColor1.getGreen() * ratio);
		final float b = (rgbColor.getBlue() * inverseRatio) + (rgbColor1.getBlue() * ratio);
	 ...
	 */
	public static Color calculateBestTextColor(final int backgroundColor) {
		// sRGB [r, g, b]
		RgbColor rgbColor=new RgbColor();
		rgbColor.setRed(backgroundColor);
		rgbColor.setBlue(backgroundColor);
		rgbColor.setGreen(backgroundColor);
		final float[] preConversionValues = {((float) rgbColor.getRed()) / 255,
				((float) rgbColor.getGreen()) / 255,
				((float) rgbColor.getBlue()) / 255};
